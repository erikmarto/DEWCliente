$(function(){
	...
	function addToCart(itemID){
		...
		*/
		...
		if ($("#cart_items").children().is("#c"+itemID)){
			...
			$(copia).prepend($minus, $add, $delete, $cantidad); //agregamos los enlaces de eliminar, sumar y restar item
		}
	...
	function deleteFromCart(itemID, copia){
		...
		if (!animando){
			animando = true;
			$(copia).effect ("explode", { mode : "hide" }, 600, function(){
				animando = false;
				compras--; 
	...
	*/
	...
	function sumarCantidad(event, itemID, cantidad){
		...
		alert ();
		var $cantidad = $("#c"+itemID).find(".cantidad"); 
